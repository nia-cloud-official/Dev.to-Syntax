...
async function welcomeAndPick() {
    const choice = await vscode.window.showInformationMessage('Dev.to Syntax is active. Choose your look:', 'Pick Theme');
...
async function pickAndApplyTheme() {
    ...
    if (!/Fira Code/i.test(fontFamily)) {
        ...
        const action = await vscode.window.showInformationMessage('Tip: For ligatures like dev.to, install the Fira Code font.', open, dismiss);
        if (action === open) {
